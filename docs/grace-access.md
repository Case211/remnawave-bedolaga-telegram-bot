...
По истечении срока grace бот возвращает аккаунту лимит и сквады из снимка, но **статус не трогает**:
у истёкшей подписки его выводит сама панель (EXPIRED), а `DISABLED` означает «отключена
администратором» — именно так его импортирует бот, и кабинет после этого отказывает в продлении.
Пока панель ещё держит аккаунт `ACTIVE` с прошедшей датой (её планировщик срабатывает в пределах
полуминуты), сессия остаётся в состоянии `restoring` и закрывается следующим проходом, когда панель
покажет `EXPIRED`. Причина завершения — `timeout`.

Дата в панели при этом остаётся датой конца grace: прошедшую дату `PATCH /api/users` не принимает,
а вернуть настоящую нельзя. Бот запоминает её на подписке (`subscriptions.grace_tail_expire_at`),
и импорт «панель — истина» (полная синхронизация, вебхук `user.modified`, карточка пользователя),
увидев в панели ровно эту дату, дату и статус подписки не трогает. Настоящее продление в панели
даёт другую дату и импортируется как обычно. Без этого истёкшая подписка «истекала» заново в конец
grace, воркер видел свежее истечение и выдавал grace снова — бесконечно (стенд, 2026-09-14).

Ранний откат (`restore-all`, конфликт биллинга посреди grace) застаёт дату grace ещё в будущем —
тогда действует общее правило гашения: в панель уходит ближайший допустимый момент (сейчас плюс
пять минут), и панель гасит аккаунт сама. Восстановление `LIMITED`-снимка идёт в два шага: сначала
лимит и дата, потом, когда панель снова выведет `LIMITED`, — сквады.

## Уведомления

О каждой выдаче и завершении grace бот сообщает админам в чат уведомлений (категория «Продления»):
кому, почему (срок или трафик), сколько выдано и до какого срока, а при завершении — чем кончилось:
продлением, истечением срока, сливом, отзывом или конфликтом с панелью (с текстом ошибки). Человеку
приходит сообщение в бота: подписка закончилась, но на N часов оставлен доступ только к Telegram и
M ГБ, чтобы продлить, — с кнопкой продления; и второе, когда доступ закрылся без оплаты. Выключатели:
`GRACE_ACCESS_NOTIFY_ADMINS` и `GRACE_ACCESS_NOTIFY_USER` (раздел «Grace-доступ» на странице настроек).
